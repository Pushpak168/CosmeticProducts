...
require(quanteda)
Mydata <- read.csv("~/Desktop/NYU/Business Analytics/Data Sets/CSVDump.csv" , header = TRUE, stringsAsFactors=FALSE)
view(Mydata)
Mycorpus<- corpus(Mydata$Review.text,
                    docnames=Mydata$Sr.no,
                    docvar=data.frame(Summary = Mydata$Summary))
names(Mydata)
summary(Mydata)

Mycorpus<- toLower(Mycorpus, keepAcronyms = FALSE) #Converting to lower case

cleancorpus <- tokenize(Mycorpus, 
                        removeNumbers=TRUE,  
                        removePunct = TRUE,
...
stop_words <- c(stop_words, "one", "the", "can", "will", "get","go","much")
...
serVis(json, out.dir = 'Beauty Products', open.browser = TRUE)
